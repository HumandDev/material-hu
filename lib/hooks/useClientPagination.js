...
const useClientPagination = (items, queriedKeys, placeholder, defaultOrderBy = 'position', defaultOrder = 'asc', limitOptions = [10, 20, 30]) => {
    ...
    const searchBar = (_jsx(SearchBarController, { control: control, placeholder: placeholder, setValue: setValue }));
    const paginationController = (_jsx(PaginationController, { control: control, total: filteredItems.length, setPage: setPage, setLimit: setLimit, limitOptions: limitOptions }));
    return {
        paginatedItems,
        searchBar,
        resetForm: reset,
