...
const useServerPagination = (placeholder = '', // Depends if the searchBar is used
labelRowsPerPage = '', // Depends if paginationController is used
defaultOrderBy = 'CREATED_AT', defaultOrder = 'ASC', limitOptions = [10, 20, 30], isSurveys = false, isPeopleExperience = false) => {
    const form = useForm({
        defaultValues: {
            query: '',
            pagination: {
    ...
    const searchBar = (_jsx(SearchBarController, { control: control, setValue: setValue, placeholder: placeholder, isSurveys: isSurveys, isPeopleExperience: isPeopleExperience }));
