...
const useServerPagination = (placeholder = '', defaultOrderBy = 'CREATED_AT', defaultOrder = 'ASC', limitOptions = [10, 20, 30], isSurveys = false) => {
    const form = useForm({
        defaultValues: {
            query: '',
            pagination: {
    ...
    const setOrder = (newOrder) => setValue('order', newOrder);
    ...
    const paginationController = (total) => (_jsx(PaginationController, { control: control, total: total, setPage: setPage, setLimit: setLimit, limitOptions: limitOptions }));
