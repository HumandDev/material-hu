...
export const FormDrop = (props) => {
    var _a, _b;
    const { name, rules, maxSize: maxSizeProp, recommendedWidth = RECOMMENDED_WIDTH, recommendedHeight = RECOMMENDED_HEIGHT, getErrorMessage = () => '', altLabel = () => '', deleteLabel = () => '', helpTextLabel = () => '', linkLabel = () => '', sizeLabel = () => '', openLabel = () => '', label = () => '', accept: acceptProp, type = FormDropTypes.IMAGE, } = props;
    ...
    return (_jsx(Controller, { name: name, control: control, rules: rules, render: ({ field: { onChange, value } }) => {
            ...
            const hasValue = (((_a = dropValue === null || dropValue === void 0 ? void 0 : dropValue.url) === null || _a === void 0 ? void 0 : _a.length) && dropValue.url.length > 0) || !!(dropValue === null || dropValue === void 0 ? void 0 : dropValue.file);
            return (_jsxs(Stack, { spacing: 3, width: "100%", children: [hasValue && type !== FormDropTypes.PDF && (_jsxs(_Fragment, { children: [_jsx(Box, { component: type === FormDropTypes.IMAGE ? "img" : "video", src: dropValue.url || (dropValue.file && URL.createObjectURL(dropValue.file)), alt: altLabel(context), controls: true, sx: {
                                    width: '100%',
                                    height: 'auto',
