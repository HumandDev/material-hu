...
const getDotOffset = (size, badgeContent) => {
    if (badgeContent) {
        return {};
    }
    const translateOffset = size === 'medium' || size === 'small' ? '-0.5px' : '-4px';
    return {
        '& .MuiBadge-badge': {
...
const Avatar = (_a) => {
    ...
    return withBadge ? (_jsx(Badge, Object.assign({}, badgeProps, { anchorOrigin: { vertical: 'bottom', horizontal: 'right' }, sx: getDotOffset(size, badgeContent), 
