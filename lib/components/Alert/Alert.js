...
const Alert = (props) => {
    ...
    return (_jsxs(AlertMui, { icon: _jsx(data.icon, { color: data.color }), action: _jsxs(Stack, { sx: { flexDirection: 'row', alignItems: 'center' }, children: [action && (_jsx(Button, { onClick: action.onClick, sx: {
                        color: data.color,
                        minWidth: 'unset',
                        '&:hover': {
