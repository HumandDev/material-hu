...
const ButtonGroup = ({ labels, onChange, fullWidth = false, disableUnselect = false, fixedCheck = false, showCheckIcon = true, }) => {
    ...
    return (_jsx(MUIButtonGroup, { variant: "outlined", sx: { display: 'flex', width: fullWidth ? '100%' : 'auto' }, children: labels.map((label, index) => (_jsx(Button, { onClick: () => buttonSelection(index), sx: {
                ...
                textTransform: 'none',
            }, startIcon: showCheckIcon && selectedButton === index ? (_jsx(IconCheck, { size: 16 })) : (noIcon), endIcon: noIcon, children: label }, index))) }));
};
