...
import { AnnouncementOutlined as AnnouncementOutlinedIcon, Check as CheckIcon, ErrorOutline as ErrorIcon, InfoOutlined as InfoIcon, TipsAndUpdatesOutlined as TipsAndUpdatesOutlinedIcon, WarningAmberRounded as WarningIcon, } from '@mui/icons-material';
...
const Pills = ({ label, type = 'default', size = 'medium', hasIcon = true, sx, }) => {
    ...
    return (_jsx(Chip, { label: _jsx(Typography // variant: isSmall ? 'globalXXS' : 'globalXS', fontWeight: 'fontWeightSemiBold',
        , { sx: Object.assign({ margin: 0, padding: 0, lineHeight: 'normal', color: pillProps.fontColor, fontSize: isSmall ? '12px' : '14px', fontWeight: 500 }, sx), children: label }), size: size, icon: hasIcon && pillProps.icon ? (_jsx(pillProps.icon, { sx: {
