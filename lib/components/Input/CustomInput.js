...
const CustomInput = ({ value, onChange, placeholder, inputRef, maxLength, success, multiline = false, startAdornment, sxInput, minRows = 5, disabled, }) => {
    var _a, _b, _c, _d;
    const { focused, error } = useFormControl() || {};
    const hastEndAdornment = success || error || (focused && value.length > 0);
    const theme = useTheme();
    return (_jsx(OutlinedInput, { inputRef: inputRef, inputProps: { maxLength }, multiline: multiline, minRows: minRows, startAdornment: startAdornment, endAdornment: hastEndAdornment && (_jsxs(InputAdornment, { sx: { alignSelf: multiline ? 'flex-start' : 'center' }, position: "end", children: [focused && (_jsx(IconX, { size: 20, onMouseDown: e => {
                ...
                : colorPalette.hugoBackground.neutralBgTerciary, '& fieldset': {
