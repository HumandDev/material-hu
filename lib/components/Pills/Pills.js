...
const Pills = ({ label, type = 'error', size = 'medium', hasIcon = true, sx, }) => {
    const theme = useTheme();
    const pillProps = getPillProps(type, theme.palette);
    const isSmall = size === 'small';
    return (_jsx(Chip, { label: _jsx(Typography, { variant: isSmall ? 'globalXXS' : 'globalXS', fontWeight: 'fontWeightSemiBold', sx: Object.assign({ margin: 0, padding: 0, lineHeight: 'normal', color: pillProps.fontColor }, sx), children: label }), size: size, icon: hasIcon && pillProps.icon ? (_jsx(pillProps.icon, { style: {
