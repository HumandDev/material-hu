...
import { getColorsVariant, getSizeInPixels, } from '../Avatar/Avatar';
const MAX_AVATARS = 4;
...
const AvatarGroup = (_a) => {
    var { size = 'medium', max = MAX_AVATARS } = _a, props = __rest(_a, ["size", "max"]);
    const theme = useTheme();
    const sizeInPixels = getSizeInPixels(size);
    const colorsVariant = getColorsVariant('default', theme.palette);
    return (_jsx(AvatarGroupMui, Object.assign({}, props, { sx: Object.assign(Object.assign({ justifyContent: 'start' }, props.sx), { '& .MuiAvatar-root': {
                height: sizeInPixels,
                width: sizeInPixels,
                position: 'static',
            } }), slotProps: {
        ...
        }, max: max, renderSurplus: formatSurplus })));
};
