...
function applyPagination<T>(items: T[], page: number, limit: number) {
  return items.slice(page * limit, page * limit + limit);
}

const useTableSorting = <T extends unknown>(
      ...
      [
        i => {
          const value = valueSelector
            ? valueSelector(i)
            : (i as Record<string, any>)[orderBy];
...
const useClientPagination = <TData extends object>(
  options: ClientPaginationOptions<TData>,
) => {
  const {
    items,
    queriedKeys,
  ...
  const { TableSortingHeader, sortFunction } = useTableSorting<TData>(
    defaultOrderBy,
    defaultOrder,
  );
  ...
  const filteredItems = useMemo(
    () => applyFilters(sortedItems, query, queriedKeys),
    [sortedItems, query],
  );
  const paginatedItems = useMemo(
    () => applyPagination(filteredItems, pagination.page, pagination.limit),
